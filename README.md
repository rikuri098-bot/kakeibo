...
- **複数ユーザー対応**：`users` テーブルで各ユーザーが独立した家計簿を持つ。新規登録（自動ログイン・デフォルトカテゴリ自動作成）、すべてのデータ（支出・予算・カテゴリ・ショートカット・設定）を `user_id` で完全分離。パスワードは bcrypt ハッシュを `users` テーブルに保存
- **ログイン/新規登録**：Flask-Login によるセッション認証（未ログイン時は `/login` へ自動リダイレクト）。`/register` から新規登録
- **CSVエクスポート**：月別ビューから選択中の月を「家計簿_YYYY年MM月.csv」でダウンロード（UTF-8 BOM・Excel対応）
- **設定画面（⚙）**：右上の歯車から カテゴリ管理 / 通知設定 / パスワード変更 をタブで操作
- **ショートカット**：絵文字アイコン設定、編集モーダル、ドラッグ&ドロップ並び替え、よく使う支出トップ3の自動提案
...
- **予算管理**：`budgets` テーブルで月の合計予算・カテゴリ別予算を設定（upsert）。ホーム最上部の通知エリアに予算超過（赤）／残りわずか（黄・しきい値%）／残り予算（常時表示・任意、「詳細→」で予算セクションへスクロール）。予算詳細セクションにカテゴリ別プログレスバー（超過は赤）。月別タブにも予算カード（予算/使用/残り/達成率）
- **デフォルト予算＆繰り越し**：`is_default`（month=NULL）でデフォルト予算を保存すると毎月自動適用。月次で個別変更も可能。全体予算の余り・超過を翌月へ繰り越すオプション（予算モーダルと⚙通知タブで同期）
- **PWA**：`manifest.json`／Service Worker／💰アイコン（192・512）でホーム画面に追加可能（iOS Safari対応）
- **年月ジャンプ**：月別/年間タブの年月ラベルをタップして任意の年月へジャンプ
- **CSVエクスポート / グラフ**：折れ線（上）→円グラフ（下）の順で統一。月別はCSV出力可
